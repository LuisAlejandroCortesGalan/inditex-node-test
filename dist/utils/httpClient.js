...
class HttpClient {
  constructor(baseURL, timeout = config_1.default.requestTimeout) {
    ...
    this.cache = new Map();
    this.cacheConfig = {
      defaultTtl: config_1.default.cacheTtl,
  ...
  getFromCache(key) {
    ...
    if (now - cached.timestamp > effectiveTtl) {
      ...
      logger_1.default.debug(`Cache expired for ${key}`);
      return null;
    }
    return cached.data;
  }
  addToCache(key, data) {
    ...
    logger_1.default.debug(`Added to cache: ${key}`);
  }
  async executeWithRetry(fn, options) {
      ...
      try {
        if (attempt > 0) {
          ...
          await new Promise(resolve =>
            setTimeout(resolve, config_1.default.retryDelay * Math.pow(2, attempt - 1))
          );
        }
    ...
    if (lastError) {
      if (
        (axios_1.default.isAxiosError(lastError) && lastError.code === 'ECONNABORTED') ||
  ...
  invalidateCache(pattern) {
    ...
    logger_1.default.debug(`Invalidated ${count} cache entries matching pattern: ${pattern}`);
  }
}
